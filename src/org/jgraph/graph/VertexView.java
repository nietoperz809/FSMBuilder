...
public class VertexView extends AbstractCellView
{
    ...
    {
        if (GraphConstants.isSizeable(getAllAttributes()) && !GraphConstants.isAutoSize(getAllAttributes()) && context.getGraph().isSizeable())
        {
     ...
     */
    public static transient int[] defaultCursors = new int[]{Cursor.NW_RESIZE_CURSOR, Cursor.N_RESIZE_CURSOR, Cursor.NE_RESIZE_CURSOR, Cursor.W_RESIZE_CURSOR, Cursor.E_RESIZE_CURSOR, Cursor.SW_RESIZE_CURSOR, Cursor.S_RESIZE_CURSOR, Cursor.SE_RESIZE_CURSOR};

    /**
     * Array that holds the cursors for the different control points.
     */
    public static transient int[] xCursors = new int[]{Cursor.W_RESIZE_CURSOR, 0, Cursor.E_RESIZE_CURSOR, Cursor.W_RESIZE_CURSOR, Cursor.E_RESIZE_CURSOR, Cursor.W_RESIZE_CURSOR, 0, Cursor.E_RESIZE_CURSOR};

    /**
     * Array that holds the cursors for the different control points.
     */
    public static transient int[] yCursors = new int[]{Cursor.N_RESIZE_CURSOR, Cursor.N_RESIZE_CURSOR, Cursor.N_RESIZE_CURSOR, 0, 0, Cursor.S_RESIZE_CURSOR, Cursor.S_RESIZE_CURSOR, Cursor.S_RESIZE_CURSOR};
    ...
    public static class SizeHandle implements CellHandle, Serializable
    {
        ...
        public void paint(Graphics g)
        {
            ...
            for (int i = 0; i < r.length; i++)
            {
                ...
                {
                    g
                            .fill3DRect((int) r[i].getX(), (int) r[i].getY(), (int) r[i].getWidth(), (int) r[i]
                                    .getHeight(), true);
                }
            }
        ...
        public void mouseDragged(MouseEvent event)
        {
            ...
            if (offgraphics != null || !graph.isXorEnabled())
            {
                ...
                dirty.setFrame(dirty.getX() - border2, dirty.getY() - border2, dirty.getWidth() + border, dirty.getHeight() + border);
        ...
        protected Rectangle2D computeBounds(MouseEvent event)
        {
            ...
            if (isConstrainedSizeEvent(event) || GraphConstants.isConstrained(vertex.getAllAttributes()))
            {
