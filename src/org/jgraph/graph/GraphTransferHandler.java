...
public class GraphTransferHandler extends TransferHandler
{
        ...
        try
        {
            if (comp instanceof JGraph)
            {
                ...
                if (t.isDataFlavorSupported(GraphTransferable.dataFlavor) && graph.isEnabled())
                {
                    ...
                    {
                        int mod = (graph.getUI().getDropAction() == TransferHandler.COPY) ? InputEvent.CTRL_MASK : 0;
                        graph.getUI().getHandle().mouseReleased(new MouseEvent(comp, 0, 0, mod, p.x, p.y, 1, false));
                        return false;
                    }
                    ...
                    if (!allInModel || p == null || alwaysReceiveAsCopyAction || graph.getUI().getDropAction() == TransferHandler.COPY)
                    {
                        ...
                        if (nested != null)
                        {
                            ...
                            {
                                Point2D insertPoint = getInsertionOffset(graph, inCount, bounds);
                                if (insertPoint != null)
                                {
                                    dx = insertPoint.getX();
                                    dy = insertPoint.getY();
                                    ...
                                    if (!model.isEdge(cells[i]) && !model.isPort(cells[i]))
                                    {
                                                        ...
                                                        .getParent(cells[i])))
                                        {
                                            ...
                                            if (view != null && !view.isLeaf())
                                            {
                                                ...
                                                GraphLayoutCache
                                                        .translateViews(new CellView[]{view}, dx, dy);
                                            }
                                            else
                                            {
                                                tmp.setFrame(p.getX(), p.getY(), tmp
                                                        .getWidth(), tmp.getHeight());
                                            }
    ...
    /**
     * @param alwaysReceiveAsCopyAction The alwaysReceiveAsCopyAction to set.
     */
