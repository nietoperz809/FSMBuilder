...
public class AttributeMap extends Hashtable implements Cloneable
{
     ...
     * @param loadCapacity    the load factor of the hashtable.
     */
     ...
     *
     * @param map the input map to copy
     */
     ...
     *
     * @param p the point whose values the new point are to be based on
     ...
     *
     * @param x the x-coordinate position of the new point
     * @param y the y-coordinate position of the new point
     ...
     * values as those passed in
     *
     * @param x the x-coordinate position of the new rectangle
     * @param y the y-coordinate position of the new rectangle
     ...
     *
     * @param pt the position of the new rectangle
     ...
     *
     * @param pt   the position of the new rectangle
     * @param size the length of both sides of the rectangle
     ...
     * @param w     the width of the new rectangle
     * @param h     the height of the new rectangle
     * @param grow1 the amount both dimensions are to be increased by and the
     *              position coorindates of the rectangle are to be decreased by
     * @param grow2 the additional amount by which both dimensions are to be
     *              increased by
     * @return a new rectangle at the coordinates and of the dimensions passed
     *         in
     */
    ...
    public AttributeMap applyMap(Map change)
    {
        ...
        if (change != null)
        {
            ...
            while (it.hasNext())
            {
                ...
                if (!key.equals(GraphConstants.REMOVEALL) && !key.equals(GraphConstants.REMOVEATTRIBUTES) && !key.equals(GraphConstants.VALUE))
                {
     ...
     *
     * @param key   the map key whose value is to be altered
     * @param value the new value to be applied to the specified key
     ...
     *
     * @param points the points to be cloned
    ...
    public void scale(double sx, double sy, Point2D origin)
    {
        ...
        if (bounds != null)
        {
            ...
            p.setLocation(origin.getX() + Math.round((p.getX() - origin.getX()) * sx), origin
                    .getY() + Math.round((p.getY() - origin.getY()) * sy));
        ...
        if (points != null)
        {
            ...
            while (it.hasNext())
            {
                ...
                if (obj instanceof Point2D)
                {
                    ...
                    p.setLocation(origin.getX() + Math.round((p.getX() - origin.getX()) * sx), origin.getY() + Math.round((p.getY() - origin.getY()) * sy));
                    // Move Point
                    loc.setLocation(p);
                }
            }
