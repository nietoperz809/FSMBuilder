...
public class EdgeRenderer extends JComponent implements CellViewRenderer, Serializable
{
    ...
    protected void installAttributes(CellView view)
    {
        ...
        lineStyle = (routing != null && view instanceof EdgeView) ? routing
                .getPreferredLineStyle((EdgeView) view) : Edge.Routing.NO_PREFERENCE;
    ...
    public void paint(Graphics g)
    {
        if (view.isLeaf())
        {
            ...
            if (edgeShape != null)
            {
                ...
                g
                        .setFont((extraLabelFont != null) ? extraLabelFont : getFont());
                Object[] labels = GraphConstants.getExtraLabels(view
                        .getAllAttributes());
                JGraph graph = (JGraph) this.graph.get();
                if (labels != null)
                {
                    for (int i = 0; i < labels.length; i++)
                    {
                        paintLabel(g, graph.convertValueToString(labels[i]), getExtraLabelPosition(view, i), false || !simpleExtraLabels);
                    }
                }
            ...
            if (lineStyle == GraphConstants.STYLE_BEZIER && n > 2)
            {
                ...
                for (int i = 2; i < n - 1; i++)
                {
                    ...
                    view.sharedPath.curveTo((float) b0.getX(), (float) b0
                            .getY(), (float) b1.getX(), (float) b1.getY(), (float) p[i].getX(), (float) p[i].getY());
                }
