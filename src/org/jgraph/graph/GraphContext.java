...
public class GraphContext implements CellMapper
{
    ...
    public GraphContext(JGraph graph, Object[] cells)
    {
        ...
        cellSet = new HashSet();
        for (Object allCell : allCells)
        {
            if (graphLayoutCache.isVisible(allCell))
            {
                cellSet.add(allCell);
                if (!model.isPort(allCell))
                {
                    cellCount++;
                }
            }
     ...
     * @return true if no object has no cells
     */
    ...
    public CellView[] createTemporaryContextViews(Set cellSet)
    {
        ...
        do
        {
            ...
            while (it.hasNext())
            {
                ...
                if (!cellSet.contains(obj) && graphLayoutCache.isVisible(obj) && edge != null && (PREVIEW_EDGE_GROUPS || edge.isLeaf()))
                {
    ...
    public CellView getMapping(Object cell, boolean create)
    {
        if (cell != null)
        {
            ...
            else if (contains(cell) || (graph.getModel().isPort(cell) && create && graph
    ...
    public ConnectionSet disconnect(CellView[] cells)
    {
        ...
        for (int i = 0; i < cells.length; i++)
        {
            if (cells[i] instanceof EdgeView)
            {
                ...
                {
                    if (port != null && GraphConstants.isDisconnectable(port
                            .getParentView().getAllAttributes()) && !contains(port.getCell()))
                    {
                        view.setSource(null);
