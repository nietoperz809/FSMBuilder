...
public class DefaultGraphSelectionModel implements GraphSelectionModel, Cloneable, Serializable
{
    ...
    public void setSelectionMode(int mode)
    {
        ...
        selectionMode = mode;
        if (selectionMode != GraphSelectionModel.MULTIPLE_GRAPH_SELECTION && selectionMode != GraphSelectionModel.SINGLE_GRAPH_SELECTION)
        {
     ...
     *
     * @param cell the cell to add to the current selection
     */
     ...
     *
     * @param cell the cell to remove from the current selection
     */
            ...
            for (int i = 0; i < cells.length; i++)
            {
                if (cells[i] != null)
                {
                    ...
                    if (removed)
                    {
                        ...
                        {
                            change
                                    .addElement(new CellPlaceHolder(parent, false));
                        }
                    }
                }
            }
    ...
    {
        AttributeMap attrs = graph.getAttributes(cell);
        if (!isCellSelected(cell) && graph.getGraphLayoutCache().isVisible(cell) && (attrs == null || GraphConstants.isSelectable(attrs)))
        {
            GraphModel model = graph.getModel();
