...
public class EdgeView extends AbstractCellView
{
    ...
    {
        return (getSource() != null && getSource() == getTarget()) || (sourceParentView != null && sourceParentView == targetParentView) || (sourceParentView != null && getTarget() != null && getTarget()
                .getParentView() == sourceParentView) || (targetParentView != null && getSource() != null && getSource()
    ...
    public Point2D getPoint(int index)
    {
        ...
        {
            return sourceParentView.getPerimeterPoint(this, getCenterPoint(sourceParentView), getNearestPoint(index == 0));
        }
        else if (index == getPointCount() - 1 && targetParentView != null)
        {
            return targetParentView.getPerimeterPoint(this, getCenterPoint(targetParentView), getNearestPoint(index == 0));
        }
    ...
    protected Point2D getNearestPoint(boolean source)
    {
        if (getPointCount() == 2)
        {
            if (source && target instanceof PortView && GraphConstants.getOffset(target.getAllAttributes()) != null)
            {
    ...
    public static class EdgeHandle implements CellHandle, Serializable
    {
        ...
        public void paint(Graphics g)
        {
            ...
            for (int i = 0; i < r.length; i++)
            {
                ...
                if (port != null || (i == 0 && edge.getSourceParentView() != null) || (i == r.length - 1 && edge.getTargetParentView() != null))
                {
                    g.setColor(graph.getLockedHandleColor());
                    ...
                    if (tmp != null)
                    {
                        g.drawLine((int) r[i].getX() + 1, (int) r[i].getY() + 1, (int) (r[i].getX() + r[i].getWidth()) - 3, (int) (r[i].getY() + r[i].getHeight()) - 3);
                        g.drawLine((int) r[i].getX() + 1, (int) (r[i].getY() + r[i].getHeight()) - 3, (int) (r[i].getX() + r[i].getWidth()) - 3, (int) r[i].getY() + 1);
                    }
                    else
                    {
                        g.drawRect((int) r[i].getX() + 2, (int) r[i].getY() + 2, (int) r[i].getWidth() - 5, (int) r[i]
                                .getHeight() - 5);
                    }
                }
            }
                        ...
                        if (edge.getSource() != portView)
                        {
                            ...
                            else
                            {
                                ...
                                graph.repaint((int) dirty.getX(), (int) dirty
                                        .getY(), (int) dirty.getWidth(), (int) dirty.getHeight());
                            }
                        }
                        return true;
                    }
        ...
        public void mouseMoved(MouseEvent event)
        {
            ...
            if (loc.contains(event.getPoint()) && graph.isMoveable() && GraphConstants.isMoveable(edge.getAllAttributes()))
            {
                graph.setCursor(new Cursor(Cursor.HAND_CURSOR));
                event.consume();
            }
            if (extraLabelLocations != null && graph.isMoveable() && GraphConstants.isMoveable(edge.getAllAttributes()))
            {
                for (int i = 0; i < extraLabelLocations.length; i++)
                {
        ...
        public void mousePressed(MouseEvent event)
        {
            ...
            // Detect hit on label
            if (!isEditing() && graph.isMoveable() && GraphConstants.isMoveable(edge.getAllAttributes()) && loc != null && loc.contains(x, y) && !isAddPointEvent(event) && !isRemovePointEvent(event) && graph.getEdgeLabelsMovable())
            {
            ...
            if (extraLabelLocations != null && !isEditing() && graph.isMoveable() && graph.getEdgeLabelsMovable() && GraphConstants.isMoveable(edge.getAllAttributes()))
            {
                for (int i = 0; i < extraLabelLocations.length; i++)
                {
            ...
            // Remove Point
            if (isRemovePointEvent(event) && currentPoint != null && !source && !target && bendable && (edge.getSource() == null || currentIndex > 0) && (edge.getTarget() == null || currentIndex < edge
                    .getPointCount() - 1))
            {
        ...
        public void mouseDragged(MouseEvent event)
        {
            ...
            {
                boolean disconnectable = (!source && !target) || (graph.isDisconnectable() && GraphConstants
                        .isDisconnectable(orig.getAllAttributes()));
                if (source)
                {
                    disconnectable = disconnectable && ((orig.getSource() == null && orig
                            .getSourceParentView() == null) || (orig.getSource() != null && GraphConstants
                ...
                if (target)
                {
                    disconnectable = disconnectable && ((orig.getTarget() == null && orig
                            .getTargetParentView() == null) || (orig.getTarget() != null && GraphConstants
                ...
                if (!((source && snap(true, event.getPoint())) || (target && snap(false, event.getPoint()))) && disconnectable)
                {
                    // Else Use Point
                    boolean acceptSource = source && (graph.getModel().acceptsSource(edge.getCell(), null) || graph.isPreviewInvalidNullPorts());
                    boolean acceptTarget = target && (graph.getModel().acceptsTarget(edge.getCell(), null) || graph.isPreviewInvalidNullPorts());
                    if (acceptSource || acceptTarget || !(source || target))
                    {
                        ...
                        if (isConstrainedMoveEvent(event) && currentIndex >= 0)
                        {
                            ...
                            EdgeView orig = (EdgeView) graph
                                    .getGraphLayoutCache().getMapping(edge.getCell(), false);
        ...
        protected Point2D getRelativeLabelPosition(EdgeView edge, Point2D p)
        {
            ...
            Point2D result = new Point2D.Double(((((totalLength / 2 - length - projlen) / totalLength) * -2) + 1) * GraphConstants.PERMILLE / 2, yDistance);
        ...
        public void mouseReleased(MouseEvent e)
        {
            ...
            Object target = (edge.getTarget() != null) ? edge.getTarget()
                    .getCell() : null;
            if (edgeModified && model.acceptsSource(edge.getCell(), source) && model.acceptsTarget(edge.getCell(), target))
            {
            ...
            else
            {
                ...
                {
                    Rectangle2D dirty = edge.getBounds();
                    graph.repaint((int) dirty.getX(), (int) dirty.getY(), (int) dirty.getWidth(), (int) dirty.getHeight());
                }
                edge.refresh(graph.getGraphLayoutCache(), graph.getGraphLayoutCache(), false);
            }
        ...
        protected void invalidate()
        {
            ...
            if (p != null && d != null)
            {
                ...
                loc.setFrame(p.getX() - s.getX() / 2, p.getY() - s.getY() / 2, s.getX(), s.getY());
            }
            for (int i = 0; i < r.length; i++)
            {
                p = e.getPoint(i);
                p = graph.toScreen(new Point2D.Double(p.getX(), p.getY()));
                r[i].setFrame(p.getX() - handlesize, p.getY() - handlesize, 2 * handlesize, 2 * handlesize);
            ...
            if (extraLabelLocations != null)
            {
                for (int i = 0; i < extraLabelLocations.length; i++)
                {
                    ...
                    if (p != null)
                    {
                        ...
                        {
                            Point2D s = graph.toScreen(new Point2D.Double(d.width, d.height));
                            extraLabelLocations[i].setFrame(p.getX() - s.getX() / 2, p.getY() - s.getY() / 2, s.getX(), s
                                    .getY());
                        }
                    }
                }
