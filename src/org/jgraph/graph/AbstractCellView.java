...
public abstract class AbstractCellView implements CellView, Serializable
{
     ...
     *
     * @param cell the model object this view will represent
     */
     ...
     * first.
     *
     * @param cache                the graph model to be used
     * @param mapper               the cell mapper to be used
     * @param createDependentViews whether or not to create a view if one does not already exist
     */
     ...
     *
     * @param views the cell views whose descendants are to be returned
     ...
     * update.
     *
     * @param change a map of attribute changes to apply
     ...
     *
     * @param views the views for whom the bounding box is to be determined
     ...
     * scaleView.
     *
     * @param bounds the new bounds for this cell view
     */
    public void setBounds(Rectangle2D bounds)
    {
        ...
        else
        {
            localBounds.setFrame(localBounds.getX(), localBounds.getY(), bounds
                    .getWidth() - pe.getX() + p0.getX(), bounds.getHeight() - pe.getY() + p0.getY());
        }
    ...
    protected void updateGroupBounds()
    {
        ...
        if (r != null)
        {
            r.setFrame(r.getX() - groupBorder, r.getY() - groupBorder, r
                    .getWidth() + 2 * groupBorder, r.getHeight() + 2 * groupBorder);
        }
        groupBounds = r;
    }
     ...
     *
     * @param view the cell view to be included in the group bounds or not
     ...
     *
     * @param dx the x-coordinate amount to translate by
     * @param dy the y-coordinate amount to translate by
     */
     ...
     *
     * @param sx     the multiple by which the x coordinate position of the cell
     *               view is to be scaled
     * @param sy     the multiple by which the y coordinate position of the cell
     *               view is to be scaled
     * @param origin the origin point from which the scaling will calculate
     */
     ...
     *
     * @param graph the <code>JGraph</code> instance of the view
     * @param rect  the rectangle within which intersection is being checked for
     ...
     *
     * @param graph    the <code>JGraph</code> instance of the view
     * @param selected whether or not this view is selected
     * @param focus    whether or not this view is the focus
     * @param preview  whether or not it is a preview of the view
     ...
     *
     * @param context the context of this cell view (cells indirectly affected by
     *                it)
