...
public class BasicGraphUI extends GraphUI implements Serializable
{
    ...
    public static final boolean DNDPREVIEW = System.getProperty("java.version")
            .compareTo("1.4.0") < 0 || System.getProperty("java.version").compareTo("1.4.0") > 0;
    ...
    protected void installListeners()
    {
        ...
        if ((graphSelectionListener = createGraphSelectionListener()) != null && graphSelectionModel != null)
        {
            graphSelectionModel
                    .addGraphSelectionListener(graphSelectionListener);
        }
    ...
    ActionMap createActionMap()
    {
        ...
        map
                .put("selectPrevious", new GraphIncrementAction(1, "selectPrevious"));
    ...
    public void paint(Graphics g, JComponent c)
    {
        if (graph != c)
        {
            throw new InternalError("BasicGraphUI cannot paint " + c.toString() + "; " + graph + " was expected.");
        }
     ...
     *
     * @param g the graphics object to paint the overlay to
     */
     ...
     * clip bounds, if any
     *
     * @param g          the graphics object to draw the graph to
     * @param clipBounds the bounds within graph cells must intersect to be redrawn
     */
...
//		}
        ...
        if (clipBounds != null)
        {
            realClipBounds = graph.fromScreen(new Rectangle2D.Double(clipBounds.getX(), clipBounds.getY(), clipBounds
    ...
    public void paintCell(Graphics g, CellView view, Rectangle2D bounds, boolean preview)
    {
        ...
        if (view != null && bounds != null)
        {
            ...
            rendererPane.paintComponent(g, component, graph, (int) bounds
                    .getX(), (int) bounds.getY(), (int) bounds.getWidth(), (int) bounds.getHeight(), true);
        }
        // Then Paint Children
        if (!view.isLeaf())
        {
     ...
     * @param clip The clipping region to draw into
     */
     ...
     *
     * @param g         The graphics object to paint the image on.
     * @param component the component to be painted onto the background image
     * @param clip      The clipping region to draw into
     */
        ...
        if (ports != null)
        {
            ...
            for (int i = 0; i < ports.length; i++)
            {
                if (ports[i] != null)
                {
                    ...
                    bounds.setFrame(center.getX() - bounds.getWidth() / 2, center.getY() - bounds.getHeight() / 2, bounds
    ...
    protected void updateCachedPreferredSize()
    {
        ...
        preferredSize = new Dimension((int) Math.max(min.getX(), scaled.getX()), (int) Math.max(min
                .getY(), scaled.getY()));
        ...
        if (image != null)
        {
            ...
            preferredSize = new Dimension((int) Math.max(preferredSize
                    .getWidth(), imageSize.getX()), (int) Math.max(preferredSize.getHeight(), imageSize.getY()));
        }
        Insets in = graph.getInsets();
        if (in != null)
        {
            preferredSize.setSize(preferredSize.getWidth() + in.left + in.right, preferredSize.getHeight() + in.top + in.bottom);
        }
    ...
    public Dimension getPreferredSize(JComponent c)
    {
        ...
        if (graph != null)
        {
            if (pSize != null)
            {
                return new Dimension(Math.max(pSize.width, preferredSize.width), Math.max(pSize.height, preferredSize.height));
            }
    ...
    protected void completeEditing()
    {
        ...
        if (graph.getInvokesStopCellEditing() && stopEditingInCompleteEditing && editingComponent != null)
        {
        ...
        if (graph.isCellEditable(cell))
        {
            ...
            if (cellEditor.isCellEditable(event))
            {
                ...
                Point2D p = getEditorLocation(cell, editorSize, graph
                        .toScreen(new Point2D.Double(cellBounds.getX(), cellBounds.getY())));

                editingComponent.setBounds((int) p.getX(), (int) p.getY(), (int) editorSize.getWidth(), (int) editorSize
                        .getHeight());
                editingCell = cell;
                ...
                graph.paintImmediately((int) p.getX(), (int) p.getY(), (int) (visRect.getWidth() + visRect.getX() - cellBounds
                ...
                if (event instanceof MouseEvent)
                {
                    ...
                    Component activeComponent = SwingUtilities
                            .getDeepestComponentAt(editingComponent, componentPoint.x, componentPoint.y);
    ...
    protected Point2D getEditorLocation(Object cell, Dimension2D editorSize, Point2D pt)
    {
        ...
        if (view instanceof EdgeView)
        {
            ...
            if (renderer instanceof EdgeRenderer)
            {
                ...
                pt.setLocation(Math.max(0, pt.getX() - editorSize.getWidth() / 2), Math.max(0, pt.getY() - editorSize.getHeight() / 2));
            }
            graph.toScreen(pt);
        }
        return pt;
            ...
            if (timer == null)
            {
                ...
                else
                {
                    ...
                    if (scrollBar == null || !scrollBar.getValueIsAdjusting())
                    {
                        ...
                        if ((scrollBar = scrollPane.getHorizontalScrollBar()) != null && scrollBar.getValueIsAdjusting())
                        {
                            startTimer();
                        }
                        else
    ...
    public class GraphLayoutCacheHandler implements GraphLayoutCacheListener, Serializable
    {
        ...
        public void graphLayoutCacheChanged(GraphLayoutCacheEvent e)
        {
            ...
            Object[] inserted = e.getChange().getInserted();
            if (inserted != null && inserted.length > 0 && graphLayoutCache.isSelectsLocalInsertedCells() && !(graphLayoutCache.isSelectsAllInsertedCells() && !graphLayoutCache
        ...
        public void valueChanged(GraphSelectionEvent event)
        {
            ...
            if (cells != null && cells.length <= MAXCLIPCELLS)
            {
                ...
                if (r != null)
                {
                    ...
                    updateHandle();
                    Rectangle dirtyRegion = new Rectangle((int) (r.getX() - hsize), (int) (r.getY() - hsize), (int) (r.getWidth() + 2 * hsize), (int) (r.getHeight() + 2 * hsize));
    ...
    public class KeyHandler extends KeyAdapter implements Serializable
    {
        ...
        public void keyPressed(KeyEvent e)
        {
            if (graph != null && graph.hasFocus() && graph.isEnabled())
            {
                ...
                {
                    repeatKeyAction.actionPerformed(new ActionEvent(graph, ActionEvent.ACTION_PERFORMED, ""));
                    e.consume();
                }
                else
                {
    ...
    public class MouseHandler extends MouseAdapter implements MouseMotionListener, Serializable
    {
         ...
         * @param e    the mouse event triggering the edit
        ...
        protected void postProcessSelection(MouseEvent e, Object cell, boolean wasSelected)
        {
            if (wasSelected && graph.isCellSelected(cell) && e.getModifiers() != 0)
            {
                ...
                while (((nextParent = graphModel.getParent(parent)) != null) && graphLayoutCache.isVisible(nextParent))
                {
                    parent = nextParent;
                }
    ...
    public class RootHandle implements CellHandle, Serializable
    {
                        ...
                        if (cellView instanceof EdgeView)
                        {
                            ...
                            if (edgeView.getSource() == null)
                            {
                                ...
                                if (pt != null)
                                {
                                    ...
                                    {
                                        Rectangle2D.union(ret, new Rectangle2D.Double(pt
                                                .getX(), pt.getY(), 0, 0), ret);
                                    }
                                }
                            }
        ...
        public void overlay(Graphics g)
        {
            ...
            { // Paint Cached Bounds
                ...
                g.drawRect((int) cachedBounds.getX(), (int) cachedBounds.getY(), (int) cachedBounds
                        .getWidth() - 2, (int) cachedBounds.getHeight() - 2);
            ...
            if (!event.isConsumed() && graph.isMoveable())
            {
                ...
                { // Start Move if over cell
                    ...
                    if (view != null)
                    {
                        ...
                        { // this is the original RootHandle's mode.
                        ...
                        isContextVisible = contextViews != null && contextViews.length < MAXCELLS && (!event.isControlDown() || !graph
                                .isCloneable());
                        event.consume();
                    }
                }
        ...
        protected CellView findViewForPoint(Point2D pt)
        {
            ...
            Rectangle2D r = new Rectangle2D.Double(pt.getX() - snap, pt.getY() - snap, 2 * snap, 2 * snap);
            for (int i = 0; i < views.length; i++)
            {
        ...
        protected CellView findUnselectedInnermostGroup(double x, double y)
        {
            ...
            for (int i = cells.length - 1; i >= 0; i--)
            {
                ...
                if (view != null && !view.isLeaf() && !context.contains(view.getCell()) && view.getBounds().contains(x, y))
                {
                    return view;
                }
            }
        ...
        protected void startDragging(MouseEvent event)
        {
            ...
            {
                int action = (event.isControlDown() && graph.isCloneable()) ? TransferHandler.COPY : TransferHandler.MOVE;
                TransferHandler th = graph.getTransferHandler();
        ...
        public void mouseDragged(MouseEvent event)
        {
            ...
            if (event != null && !event.isConsumed())
            {
                ...
                { // Move Cells
                    ...
                    Point2D point = new Point2D.Double(ep.getX() - _mouseToViewDelta_x, ep.getY() - _mouseToViewDelta_y);
                    ...
                    if (isMoving || Math.abs(dx) > thresh || Math.abs(dy) > thresh)
                    {
                        ...
                        if (dx != 0 || dy != 0)
                        {
                            ...
                            isContextVisible = (!event.isControlDown() || !graph
                                    .isCloneable()) && contextViews != null && (contextViews.length < MAXCELLS);
                            blockPaint = false;
                            if (constrained && cachedBounds == null)
                            {
                                ...
                                for (int i = 0; i < all.length; i++)
                                {
                                    ...
                                    all[i].changeAttributes(graph
                                            .getGraphLayoutCache(), (AttributeMap) attr.clone());
                            ...
                            if (cachedBounds != null)
                            {
                                ...
                                cachedBounds.setFrame(cachedBounds.getX() + dx * scale, cachedBounds.getY() + dy * scale, cachedBounds.getWidth(), cachedBounds.getHeight());
                                if (dirty != null)
                                {
                                    dirty.add(cachedBounds);
                                }
                            ...
                            if (graph.isAutoResizeGraph() && (event.getX() > graph.getWidth() - SCROLLBORDER || event.getY() > graph
                                    .getHeight() - SCROLLBORDER))
                            {
                                ...
                                if (view != null)
                                {
                                    if (view.contains(event.getPoint()))
                                    {
                                        ...
                                        {
                                            preferredSize.width = Math.max(preferredSize.width, (int) view.getWidth()) + SPREADSTEP;
                                            spread = true;
                                        }
                                        if (view.y + view.height - event.getPoint().y < SCROLLBORDER)
                                        {
                                            preferredSize.height = Math.max(preferredSize.height, (int) view.getHeight()) + SPREADSTEP;
                                            spread = true;
                                        }
                                        if (spread)
                                        {
                            ...
                            targetGroup = null;
                            if (graph.isMoveIntoGroups() && (ignoredRegion == null || !ignoredRegion
                                    .intersects(AbstractCellView
                                            .getBounds(views))))
                            {
                                targetGroup = (event.isControlDown()) ? null : findUnselectedInnermostGroup(snapCurrent.getX() / scale, snapCurrent.getY() / scale);
                                if (targetGroup == ignoreTargetGroup)
                                {
                                    targetGroup = null;
                                }
                            }
                            if (!snapCurrent.equals(snapStart) && (offgraphics != null || !blockPaint) && !spread)
                            {
                                if (graph.isXorEnabled())
                                {
                                    overlay(g);
                                    ...
                                    .isXorEnabled()))
                            {
                                ...
                                dirty.setFrame(dirty.getX() - border2, dirty
                                        .getY() - border2, dirty.getWidth() + border, dirty.getHeight() + border);
                                ...
                                if (offgraphics != null)
                                {
                                    graph.drawImage((int) sx1, (int) sy1, (int) sx2, (int) sy2, (int) sx1, (int) sy1, (int) sx2, (int) sy2);
                                }
                                else
                                {
                                    graph.repaint((int) dirty.getX(), (int) dirty.getY(), (int) dirty
                                            .getWidth() + 1, (int) dirty.getHeight() + 1);
                                }
                            }
                        }
                    } // end if (isMoving or ...)
        ...
        protected Point2D constrainDrag(MouseEvent event, double totDx, double totDy, double dx, double dy)
        {
            ...
            {
                if (!graph.isMoveBelowZero() && last != null && initialLocation != null && start != null)
                {
                ...
                if (!graph.isMoveBeyondGraphBounds() && last != null && initialLocation != null && start != null)
                {
                    Rectangle2D graphBounds = graph.getBounds();
            ...
            try
            {
                if (event != null && !event.isConsumed())
                {
                    ...
                    else if (isMoving && !event.getPoint().equals(start))
                    {
                        ...
                        if (event.isControlDown() && graph.isCloneable())
                        { // Clone
                            // Cells
                            ...
                            for (int i = 0; i < cells.length; i++)
                            {
                                ...
                                if (witness == null)
                                {
                                    CellView view = (CellView) hiddenMapping
                                            .get(cells[i]);
                                    if (view != null && !graphModel.isPort(view
                                            .getCell()))
                                    {
                            ...
                            cells = graphLayoutCache.insertClones(cells, graph
                                    .cloneCells(cells), attributes, cs, pm, 0, 0);
                        }
                        else if (graph.isMoveable())
                        { // Move Cells
                            ParentMap pm = null;
                            ...
                            else if (graph.isMoveOutOfGroups() && (ignoreTargetGroup != null && !ignoreTargetGroup
                                    .getBounds().intersects(AbstractCellView
                                    .getBounds(views))))
                            {
    ...
    public class PropertyChangeHandler implements PropertyChangeListener, Serializable
    {
        public void propertyChange(PropertyChangeEvent event)
        {
            if (event.getSource() == graph)
            {
                ...
                if (changeName.equals(JGraph.GRID_VISIBLE_PROPERTY) || changeName.equals(JGraph.GRID_SIZE_PROPERTY) || changeName.equals(JGraph.GRID_COLOR_PROPERTY) || changeName.equals(JGraph.HANDLE_COLOR_PROPERTY) || changeName
                        .equals(JGraph.LOCKED_HANDLE_COLOR_PROPERTY) || changeName.equals(JGraph.HANDLE_SIZE_PROPERTY) || changeName.equals(JGraph.PORTS_VISIBLE_PROPERTY) || changeName.equals(JGraph.ANTIALIASED_PROPERTY))
                {
                    graph.repaint();
                }
     ...
     *
     * @param snapSelectedView specifies if the snap-to-grid mode should be applied during a
