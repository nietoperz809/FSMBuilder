        ...
        implements Scrollable, Accessible, Serializable
{
     ...
     *
     * @param cache the <code>GraphLayoutCache</code> to use as the view
     */
     ...
     *
     * @param model the <code>GraphModel</code> to use as the data model
     * @param cache the <code>GraphLayoutCache</code> to use as the cache
     */
     ...
     *
     * @param model the <code>GraphModel</code> to use as the data model
     * @param mh    the <code>BasicMarqueeHandler</code> to use as the marquee
     *              handler
     */
     ...
     *
     * @param model       the <code>GraphModel</code> to use as the data model
     * @param layoutCache the <code>GraphLayoutCache</code> to use as the cache
     * @param mh          the <code>BasicMarqueeHandler</code> to use as the marquee
     *                    handler
     */
    ...
    public CellView getTopmostViewAt(double x, double y, boolean reverse, boolean leafsOnly)
    {
        ...
        for (int i = (reverse) ? cells.length - 1 : 0; i >= 0 && i < cells.length; i += (reverse) ? -1 : +1)
        {
            CellView view = getGraphLayoutCache().getMapping(cells[i], false);
            if (view != null && (!leafsOnly || view.isLeaf()) && ((view.isLeaf() && view.intersects(this, r)) || (!view
    ...
    {
        CellView[] selectables = getGraphLayoutCache().getMapping(getSelectionModel().getSelectables(), false);
    ...
    public CellView getNextViewAt(CellView[] cells, CellView c, double x, double y, boolean leafsOnly)
    {
        if (cells != null)
        {
            Rectangle2D r = fromScreen(new Rectangle2D.Double(x - tolerance, y - tolerance, 2 * tolerance, 2 * tolerance));
            ...
            {
                if (cells[i] != null && (!leafsOnly || cells[i].isLeaf()) && cells[i].intersects(this, r))
                {
    ...
    public PortView getPortViewAt(double x, double y, int tolerance)
    {
        double sx = x / scale;
        double sy = y / scale;
        Rectangle2D r = new Rectangle2D.Double(sx - tolerance, sy - tolerance, 2 * tolerance, 2 * tolerance);
     ...
     * port.
     *
     * @param cell the cell whose port is to be obtained
     ...
     *
     * @param d a dimension in screen coordinates to snap to.
    ...
    public Dimension2D snap(Dimension2D d)
    {
        ...
        {
            double sgs = gridSize * getScale();
            d.setSize(1 + Math.round(Math.round(d.getWidth() / sgs) * sgs), 1 + Math.round(Math.round(d.getHeight() / sgs) * sgs));
        }
        return d;
    }
    ...
    public Rectangle2D toScreen(Rectangle2D rect)
    {
        ...
        rect.setFrame(rect.getX() * scale, rect.getY() * scale, rect.getWidth() * scale, rect.getHeight() * scale);
        return rect;
    }
    ...
    public void updateAutoSize(CellView view)
    {
        if (view != null && !isEditing())
        {
            Rectangle2D bounds = (view.getAttributes() != null) ? GraphConstants
                    .getBounds(view.getAttributes()) : null;
     ...
     *
     * @param edgeLabelsMovable true if edge labels may be dragged
     */
     ...
     *
     * @param mode The current grid view mode. Valid values are <CODE>
     ...
     *
     * @param flag a boolean value, true if cell selection is enabled
     */
     ...
     *
     * @param flag a boolean value, true if the graph allows invalid null ports
     *             during previews
     */
    ...
    public Graphics getOffgraphics()
    {
        ...
        int height = (int) viewPortBounds.getHeight() + (int) offscreenBuffer * 2;
        ...
        if ((offscreen == null || offgraphics == null || offscreenBounds == null) || !(offscreenBounds.contains(newOffscreenBuffer)))
        {
            if (offscreen != null)
            {
                offscreen.flush();
        ...
        if (offscreenDirty != null)
        {
            ...
            {
                ((Graphics2D) offgraphics).setComposite(AlphaComposite.getInstance(AlphaComposite.CLEAR, 0.0f));
            }
            toScreen(offscreenDirty);
            offscreenDirty.setRect(offscreenDirty.getX() - (getHandleSize() + 1), offscreenDirty.getY() - (getHandleSize() + 1), offscreenDirty.getWidth() + (getHandleSize() + 1) * 2, offscreenDirty.getHeight() + (getHandleSize() + 1) * 2);
            offgraphics.fillRect((int) offscreenDirty.getX(), (int) offscreenDirty.getY(), (int) offscreenDirty
    ...
    protected void createBufferedImage(int width, int height)
    {
        ...
        if (graphicsConfig != null)
        {
            try
            {
                offscreen = graphicsConfig.createCompatibleImage(width, height, (isOpaque()) ? Transparency.OPAQUE : Transparency.TRANSLUCENT);
            }
            catch (OutOfMemoryError e)
            {
                offscreen = null;
        ...
        else
        {
            try
            {
                offscreen = graphicsConfig.createCompatibleImage(width, height, (isOpaque()) ? BufferedImage.TYPE_INT_RGB : BufferedImage.TYPE_INT_ARGB);
            }
            catch (OutOfMemoryError e)
            {
                offscreen = null;
    ...
    {
        getOffgraphics();
        return getGraphics().drawImage(offscreen, (int) sx1, (int) sy1, (int) sx2, (int) sy2, (int) sx1, (int) sy1, (int) sx2, (int) sy2, this);
    }

    public boolean drawImage(Graphics g)
    {
        Rectangle rect = getBounds();
        return getGraphics().drawImage(offscreen, rect.x, rect.y, rect.x + rect.width, rect.y + rect.height, rect.x, rect.y, rect.x + rect.width, rect.y + rect.height, this);
     ...
     *
     * @param newLayoutCache the <code>GraphLayoutCache</code> that is to provide the
     *                       view-data
     */
     ...
     * selection.
     *
     * @param newMarquee the <code>BasicMarqueeHandler</code> that is to provide
     *                   marquee handling
     */
     ...
     *
     * @param newValue true means that <code>stopCellEditing</code> is invoked when
     ...
     *
     * @param e the <code>MouseEvent</code> that initiated the
            ...
            if (view != null)
            {
                ...
                if (c instanceof JComponent)
                {
                    ...
                    e = new MouseEvent(c, e.getID(), e.getWhen(), e
                            .getModifiers(), (int) (where.getX() - rect.getX()), (int) (where
                            .getY() - rect.getY()), e.getClickCount(), e.isPopupTrigger());
     ...
     * allow selections.
     *
     * @param selectionModel the <code>GraphSelectionModel</code> to use, or
     ...
     *
     * @param tsl the <code>GraphSelectionListener</code> that will be
    ...
    public void addGraphSelectionListener(GraphSelectionListener tsl)
    {
        ...
        if (listenerList.getListenerCount(GraphSelectionListener.class) != 0 && selectionRedirector == null)
        {
     ...
     *
     * @param cells an array of objects that specifies the cells to select
     */
    ...
    public void scrollCellToVisible(Object cell)
    {
        ...
        if (bounds != null)
        {
            ...
            scrollRectToVisible(new Rectangle((int) b2.getX(), (int) b2.getY(), (int) b2.getWidth(), (int) b2.getHeight()));
        }
    }
    ...
    public BufferedImage getImage(Color bg, int inset)
    {
        ...
        if (bounds != null)
        {
            ...
            if (graphicsConfig != null)
            {
                img = getGraphicsConfiguration().createCompatibleImage((int) bounds.getWidth() + 2 * inset, (int) bounds.getHeight() + 2 * inset, (bg != null) ? Transparency.OPAQUE : Transparency.BITMASK);
            }
            else
            {
                img = new BufferedImage((int) bounds.getWidth() + 2 * inset, (int) bounds.getHeight() + 2 * inset, (bg != null) ? BufferedImage.TYPE_INT_RGB : BufferedImage.TYPE_INT_ARGB);
            }
