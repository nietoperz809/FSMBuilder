/*
 * $Id: ParallelEdgeRouter.java,v 1.3 2007/08/30 10:40:45 david Exp $
 * 
...
//				System.out.println(edge+"-source promoted");
                from = nodeFrom.getPerimeterPoint(edge, AbstractCellView.getCenterPoint(nodeFrom), perimeterPoint);
            }
            else
            {
                from = edge.getSource().getParentView().getPerimeterPoint(edge, AbstractCellView.getCenterPoint(edge.getSource().getParentView()), (edge.getTarget() != null && edge.getTarget().getParentView() != null) ? AbstractCellView.getCenterPoint(edge.getTarget().getParentView()) : AbstractCellView.getCenterPoint(nodeTo));
            }
            Point2D to;
...
//				System.out.println(edge+"-target promoted");
            ...
            {
                to = edge.getTarget().getParentView().getPerimeterPoint(edge, AbstractCellView.getCenterPoint(edge.getTarget().getParentView()), from);
            }
                ...
                if (labelPos != null)
                {
                    ...
                    if (x == GraphConstants.PERMILLE / 2)
                    {
                        ...
                        if (allAttributes != null)
                        {
                            ...
                            // vary in height
                            if (lineStyle == GraphConstants.STYLE_BEZIER || lineStyle == GraphConstants.STYLE_SPLINE)
                            {
                                // TODO, sort this magic number out
                                GraphConstants.setLabelPosition(allAttributes, new Point2D.Double(x, r * edgeMiddleDeparture / 79));
                            }
                            else
                            {
